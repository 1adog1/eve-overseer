<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["Action"]) and $_POST["Action"] == "Get" and (isset($_POST["FleetID"]))) {
            ...
            if (!empty($pulledArrayData)) {
                ...
                foreach ($pulledArrayData as $throwaway => $pulledData) {
                    ...
                    if (in_array("HR", $_SESSION["AccessRoles"]) or in_array("Super Admin", $_SESSION["AccessRoles"]) or (in_array("Fleet Commander", $_SESSION["AccessRoles"]) and $pulledData["commanderid"] == $_SESSION["CharacterID"])) {
        ...
        elseif (isset($_POST["Action"]) and $_POST["Action"] == "List") {
            ...
            $toPull = $GLOBALS['MainDatabase']->prepare("SELECT fleetid, fleetname, srplevel, commanderid, commandername, starttime, endtime, peakmembers FROM fleets ORDER BY starttime DESC LIMIT :limit");
            ...
            if ($toPull->execute()) {
                ...
                while ($pulledData = $toPull->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if (in_array("HR", $_SESSION["AccessRoles"]) or in_array("Super Admin", $_SESSION["AccessRoles"]) or (in_array("Fleet Commander", $_SESSION["AccessRoles"]) and $pulledData["commanderid"] == $_SESSION["CharacterID"])) {
                        ...
                        $checkData["Fleet Data"][$pulledData["fleetid"]] = ["Name" => $pulledData["fleetname"], "SRP Level" => $pulledData["srplevel"], "Boss" => $pulledData["commandername"], "Start Date" => date("F jS, Y", $pulledData["starttime"]), "Start Time" => date("H:i:s", $pulledData["starttime"]), "Run Time" => ($pulledData["endtime"] - $pulledData["starttime"]), "Total Members" => $pulledData["peakmembers"], "timestamp" => $pulledData["starttime"], "Voltron" => "false"];
                    
                        if (strpos($pulledData["srplevel"], " (Voltron)") !== false) {
                            
                            $checkData["Fleet Data"][$pulledData["fleetid"]]["Voltron"] = "true";
